...
func TestInitialize(t *testing.T) {
	...
	if oauthClient.config.ClientID != cfg.ClientID {
		t.Errorf("Initialize() ClientID = %v, want %v", oauthClient.config.ClientID, cfg.ClientID)
	}

	if oauthClient.config.ClientSecret != cfg.ClientSecret {
		t.Errorf("Initialize() ClientSecret = %v, want %v", oauthClient.config.ClientSecret, cfg.ClientSecret)
	}

	if oauthClient.config.RedirectURL != cfg.RedirectURL {
		t.Errorf("Initialize() RedirectURL = %v, want %v", oauthClient.config.RedirectURL, cfg.RedirectURL)
	}

	if len(oauthClient.config.Scopes) != len(cfg.Scopes) {
		t.Errorf("Initialize() Scopes length = %v, want %v", len(oauthClient.config.Scopes), len(cfg.Scopes))
	}

	for i, scope := range cfg.Scopes {
		if oauthClient.config.Scopes[i] != scope {
			t.Errorf("Initialize() Scopes[%d] = %v, want %v", i, oauthClient.config.Scopes[i], scope)
		}
	}
}
...
func TestInitialize_SingletonPattern(t *testing.T) {
	...
	oauthClient1, err1 := GetOAuthClient()
	if err1 != nil {
		t.Fatalf("GetOAuthClient() error = %v", err1)
	}

	oauthClient2, err2 := GetOAuthClient()
	if err2 != nil {
		t.Fatalf("GetOAuthClient() error = %v", err2)
	}

	// Should be the same instance
	if oauthClient1 != oauthClient2 {
		t.Error("Multiple Initialize() calls should return the same client instance")
	}
}

func TestGetOAuthClient_NotInitialized(t *testing.T) {
	// Reset the singleton for testing
	resetSingleton()

	_, err := GetOAuthClient()
	if err == nil {
		t.Error("GetOAuthClient() should return error when not initialized")
	}

	expectedError := "microsoft OAuth client is not initialized"
	if err.Error() != expectedError {
		t.Errorf("GetOAuthClient() error = %v, want %v", err.Error(), expectedError)
	}
}
...
func TestClient_GenerateAuthURL(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			nonce:       "test-nonce-456",
			expectError: true,
			errorMsg:    "state parameter cannot be empty",
		},
		{
			name:        "empty nonce",
			state:       "test-state-123",
			nonce:       "",
			expectError: true,
			errorMsg:    "nonce parameter cannot be empty",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			authURL, err := oauthClient.GenerateAuthURL(tt.state, tt.nonce)

			if tt.expectError {
				if err == nil {
					t.Error("GenerateAuthURL() should return error")
				} else if err.Error() != tt.errorMsg {
					t.Errorf("GenerateAuthURL() error = %v, want %v", err.Error(), tt.errorMsg)
				}
				return
			}

			if err != nil {
				t.Errorf("GenerateAuthURL() unexpected error = %v", err)
				return
			}
			...
			if !strings.Contains(authURL, "state="+tt.state) {
				t.Error("Auth URL should contain state parameter")
			}

			if !strings.Contains(authURL, "nonce="+tt.nonce) {
...
func TestClient_ExchangeCodeForToken(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			oauthClient.config.Endpoint = oauth2.Endpoint{
				TokenURL: server.URL,
			}

			ctx := context.Background()
			token, err := oauthClient.ExchangeCodeForToken(ctx, tt.code)
...
func TestClient_GetUserProfile(t *testing.T) {
	tests := []struct {
		...
		statusCode      int
		expectError     bool
		expectedProfile *MicrosoftUserProfile
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			profile, err := oauthClient.GetUserProfile(ctx, token)
